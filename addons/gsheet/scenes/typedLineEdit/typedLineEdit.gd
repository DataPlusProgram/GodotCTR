...
func activateOptionMode(enumValues,enumStr):
	...
	if existingText.find(".") == -1:#he we try to set value to correct enum
		
		if valueIndex.has(existingText):#this would be a case where you wrote out the enumns in the sheet beforehand, doesn't seem to useful
			optionButton.select(valueIndex[existingText])
			value = enumValues[existingText]
		
		
		elif existingText.is_valid_int():#this is the more useful case where we map the index to the correct option item
			if  valueIndex.values().has(int(existingText)):
				optionButton.select(int(existingText))
				selected = true
				return
...
func _input(event):
	...
	if event is InputEventKey and event.pressed:
		if event.keycode == KEY_Z and event.ctrl_pressed:
			...
			if curText == previousText.back():
				previousText.pop_back()
				_on_typedLineEdit_text_changed(previousText.back())
...
func _on_typedLineEdit_text_changed(new_text : String) -> void:
	...
	if typeof(value) == TYPE_STRING:
		...
		if new_text.find("http") == 0:
			...
			parsePath(new_text)
		var prevCaretPos = caret_column
		
		
		if previousText != null:
			previousText.push_back(new_text)
		
		if previousText.size() >= 5:
			previousText.pop_front()
		
		text = new_text
		
		
		caret_column = prevCaretPos
		value = new_text
...
func index_pressed(index):
	var caller = self
	var textStr = menu.get_item_text(index)
	var curCol = sheet.curCol
	emit_signal("indexPressedSignal",caller,textStr,curCol)
...
func _on_child_entered_tree(node):
	...
	if !node.has_meta("uiPopup"):
		...
		node.add_item("Copy columns below")
		menu = node
		
		## connect menus index_pressed signal to local index_pressed function
		## is indexed but its name
		## headings get sent toc top.gd and main sheet rows get sent to mainsScript.gd
...
func _on_text_submitted(new_text : String):
	...
	if new_text != clean:
		_on_typedLineEdit_text_changed(new_text.strip_edges())
...
func activateArrayMode(data : Array):
	...
	var isSubList =  get_parent().name == "List"
	...
	if isSubList:
		arrayPopup.position.x += arrayPopup.size.x
