...
var curRow = -1 ## The index of the currently selected row
var curCol = -1 ## The index of currently selected column
var undoStack = []
...
func fetchLineEdit(content : Variant = &"",grow : bool =false,editable : bool=false) -> LineEdit:
...
func createLineEdit(content  = null,grow : bool =false,editable : bool=false) -> LineEdit:
	...
	if content != null:
		...
		if contentType != TYPE_STRING and contentType != TYPE_STRING_NAME:
			edit.text = str(content)
		else: 
			edit.text = content
...
var cachedLineEdit : LineEdit = null



func lineEditNew() -> LineEdit:

	if cachedLineEdit == null:
		cachedLineEdit = lineDuper.duplicate()
		cachedLineEdit.set("theme_override_colors/font_color",sheetStyle.fontColor)
		cachedLineEdit.set("theme_override_colors/cursor_color",sheetStyle.cursorColor)
		cachedLineEdit.hoverColor = sheetStyle.optionButtonHoverFontColor
		
		
		#cachedLineEdit.update.connect(cellChanged) this doesn't work(yet)
		#cachedLineEdit.updateRowHeight.connect(updateRowHeight)
		#cachedLineEdit.focusEntered.connect(focusCell)
		#cachedLineEdit.focusExit.connect(exitFocusCell)
		

	#var line : LineEdit = lineDuper.duplicate()
	var line : LineEdit = cachedLineEdit.duplicate()
	line.par = self
	...
	line.indexPressedSignal.connect(itemSubMenuItemClicked)
...
func itemSubMenuItemClicked(caller,textStr,curCol):
	var runningStr = ""
	
	if textStr == "Copy columns below":
		var colData = getColData(curCol)
		for idx in range(curRow,numRows):
			if !colData[idx].is_empty():
				runningStr += colData[idx] + "\n"
		#for columnIdx in range(curCol,cols.size()):
		#	runningStr += cols.get_child(columnIdx).text + "\n"
	DisplayServer.clipboard_set(runningStr)
	breakpoint
...
func addHeadingColumn(text,idx=-1):
		...
		heading.focus_entered.connect(focusHeadingCell.bind(idx))
...
func replaceAllMatching(findText : String,replacement : String,caseSensitive : bool):
	if findText.is_empty():
		return []
	
	var matches = []
	
	if !caseSensitive:
		findText = findText.to_lower()
	
	
		for x in cells:
			for y in x:
				var cellText = y.text
				var cellTxtLower = cellText.to_lower()
				
				if y.text.find(cellText):
					#y.text = y.text.replace(text,replacement)
					y.setData({"data":y.text.replace(cellText,replacement),"type":"str"})
					#y.setData(y.text.replace(text,replacement))
					continue
				
				if !caseSensitive:
					var txtLower = cellText.to_lower()
					
					if cellTxtLower.find(findText) != -1:
						var startPost = cellTxtLower.find(findText)
						var findPos = cellTxtLower.find(findText)
						
						var pre = cellText.substr(0,findPos)
						var post = cellText.substr(findPos+replacement.length())
				
						var newText = pre + replacement + post
						#y.text = newText
						y.setData({"data":newText,"type":"str"})
