...
func _on_replace_btton_pressed() -> void:
	if !%replaceSection.visible:
		%replaceSection.visible = true
		return
	var caseSensitive = $SearchDiag/Window/MarginContainer/v/h2/caseButton.button_pressed
	var lookForText = $SearchDiag/Window/MarginContainer/v/h/LineEdit.text
	var replacementText = %replacementText.text

	sheet.replaceAllMatching(lookForText,replacementText,caseSensitive)


func _on_meta_button_pressed() -> void:
	var internal = ["hasId","rowOrder","enumColumns","enumColumnsPrefix","colNames",]
	var data = sheet.data
	...
	var popup : Window =load("res://addons/gSheet/scenes/spreadsheet/full/scenes/metaList/metaList.tscn").instantiate()
	#populateMetaList(meta,popup.get_child(0))
	popup.dict = meta
	#for i in meta:
		#popup.addItem(i,meta[i])
	
	add_child(popup)
	popup.popup_centered(Vector2(100,100))
	
	pass # Replace with function body.

#func populateMetaList(dict : Dictionary, container : GridContainer):
	#
		#var label = Label.new()
		#var value = Label.new()
		#
		#label.text = i
		#value.text = str(dict[i])
