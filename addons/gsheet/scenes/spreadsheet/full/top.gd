...
func _ready():
	...
	$"../../../enumSelectorDialog".file_selected.connect(parseEnumsFromFile)
	$"enumPopup".enumChosen.connect(selectedItem)
...
func newChild(node):
	...
	node.connect("indexPressedSignal", index_pressed)
