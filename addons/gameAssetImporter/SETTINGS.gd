...
static func setTimeLog(tree : SceneTree, valName : String, startTime : int,stateInfo : StringName = "", categoryPath : String = "",print : bool = false) -> void:
	if !tree.has_meta("timings"):
		tree.set_meta("timings",{})
	
	
	var dict = tree.get_meta("timings")
	dict[stateInfo +":"+valName] = [Time.get_ticks_msec() - startTime,categoryPath]
	tree.set_meta("timings",dict)
	
	if print:
		print(valName + ": " + str(Time.get_ticks_msec() - startTime))
