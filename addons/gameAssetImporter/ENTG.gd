...
static func mapThreadWrapper(mapName : String,tree:SceneTree,game : String,storage : Array):
	storage.append(createMap(mapName,tree,game))
	
	

static func createMapThreaded(mapName : String,tree:SceneTree,game : String,storage : Array,priority : Thread.Priority = Thread.PRIORITY_NORMAL) -> Thread:
	
	var thread = Thread.new()
	thread.start(mapThreadWrapper.bind(mapName,tree,game,storage),priority)
	return thread
...
static func createMap(mapName : String,tree:SceneTree,game : String,blank : bool = false) -> Node:
	...
	for cache in caches:
		...
		var maps = creator.getAllMapNames()
		
		for i in maps.size():
			maps[i] = maps[i].to_lower()
		
		mapName = mapName.to_lower()
		
		if maps.has(mapName):
			if blank:
				return creator.createMapBlank(mapName)
			var map : Node = creator.createMap(mapName)
			if "loader" in map:
				map.loader = creator
				
			
			map.set_meta("gameName",t)
...
static  func fetchEntityThreadWrapper(entityStrId : StringName,params:Dictionary,tree:SceneTree,game : String ,saveToDisk : bool,storage):
	storage.append(fetchEntity(entityStrId,params,tree,game,saveToDisk))
	

static func fetchEntityThreaded(entityStrId : StringName,params:Dictionary,tree:SceneTree,game : String ,saveToDisk : bool,storage : Array,priority : Thread.Priority = Thread.PRIORITY_NORMAL):
	var thread = Thread.new()
	thread.start(fetchEntityThreadWrapper.bind(entityStrId,params,tree,game,saveToDisk,storage),priority)
	return thread

static func fetchEntity(entityStrId : StringName,params:Dictionary,tree:SceneTree,game : String ,saveToDisk : bool,hideInCache : bool = false,specificCacheParent :  Node = null) -> Node:
	entityStrId = entityStrId.to_lower()
	game = game.to_lower()
	...
	if t != null:#If we find an entity on disk
	...
	if cachedRequests.has(key) and !saveToDisk:
	...
	for cache : Node in caches:# if no entity exists in caches then we make it
		...
		if loader.getEntityDict().has(entityStrId):
			...
			var ent: Node = createEntity(entityStrId,params,entry,specificCacheParent,loader.getResourceManager(),loader.getEntityCreator())
			...
			print_debug("Request entity: %s was not found in any entity directory" % [entityStrId])
...
static func getSoundManager(tree : SceneTree) -> entgSoundManager:
...
static func createAndSaveEntityToDisk(tree,entityStrId,game) -> Node:
	...
	#var entry : Dictionary = loader.getEntityDict()[entityStrId]
	var ent: Node = createEntity(entityStrId,{},entityInfo,null,creatorScript.getResourceManager(),creatorScript.getEntityCreator())
...
static func getAllFlat(path,ext = "") -> PackedStringArray:
	var ret : PackedStringArray= []
	var all = allInDirectory(path)
	
	for i in all:
		if i.find(".") == -1:
			ret += getAllFlat(path + "/" + i,ext)
		
		else:
			if ext.is_empty():
				ret.append(path + "/" + i)
			elif i.get_extension() == ext:
				ret.append(path + "/" + i)
...
static func saveNodeAsScene(node : Node,path = "res://dbg/"):
...
static func spawn(tree : SceneTree,entityStr : String,params : Dictionary,pos,rot,game : String="",entityParentNode : Node = null,toDisk : bool = false,hideInCache =false,specifcCache : Node = null) -> Node:
	
	entityStr = entityStr.to_lower()
	game = game.to_lower()
	
	var entity : Node = fetchEntity(entityStr,params,tree,game,toDisk,hideInCache,specifcCache)
	if entity == null:
		return null
...
static func getEntityResourceEntries(loader,entityTxt,visited = []):
	var entityEntry = loader.getEntityDict()[entityTxt]
	var entries = []
	
	if entityEntry.has("sprites"):
		if !entityEntry["sprites"][0].length() == 0:
			entries.append(["sprite",entityEntry["sprites"],{"path":"sprites"}])
		
	
	if entityEntry.has("res"):
		entries += entityEntry["res"].duplicate(true)
		
	
	visited.append(entityTxt)
	
	if entityEntry.has("sourceScene"):
		var entries2 = getResourceDependsFromGenerator(loader,entityEntry["sourceScene"],visited)
		entries +=entries2
	
	if entityEntry.has("depends"):
		if typeof(entityEntry["depends"]) == TYPE_STRING:
			var t  =  loader.getEntityDict()
			var subEntry = loader.getEntityDict()[entityEntry["depends"].to_lower()]
			entries += getResourceDependsFromGenerator(loader,subEntry["sourceScene"],visited)
		
		else:
			for i in entityEntry["depends"]:
				#var subEntry = loader.getEntityDict()[i]
				entries += getEntityResourceEntries(loader,i)
				
		
	
	for e in entries:
		entryPathAdder(loader,e)
			

	return entries

static func removeDuplicateResEntries(resEntries : Array):
	breakpoint

static func entryPathAdder(loader,entry):
	if loader.resourceTypeDefaultSaveDir.has(entry[0]):
		if entry.size() <3:
			entry.append({"path":loader.resourceTypeDefaultSaveDir[entry[0]]})
		elif !entry[2].has("path"):
			breakpoint
...
static func getResourceDependsFromGenerator(loader,scenePath,visited = []):
	var scene = load(scenePath).instantiate()
	var retRes = []
	var entityDirectory = loader.getEntityDict()
	if scene == null:
		return []
		
	var generator = scene.get_node_or_null("Generator")
	
	if generator == null:
		return []
	
	if "loader" in generator:
		generator.loader = loader.getResourceManager()
	
	if generator.has_method("getResDepends"):
		retRes += (generator.getResDepends())
	

	
	if "entityDepends" in generator:
		for dependEntStrId in generator.entityDepends:
			if visited.has(dependEntStrId):
				continue
				
			
			visited.append(dependEntStrId)
			var dependSrcScene : String = entityDirectory[dependEntStrId.to_lower()]["sourceScene"]
			
			retRes += (getResourceDependsFromGenerator(loader,dependSrcScene,visited))
		
		
	if "dependantChildren" in generator:
		for scenePathStr in generator.dependantChildren:
			retRes.append(getResourceDependsFromGenerator(loader,scenePathStr,visited))
...
static func getDirsInDir(dirPath) -> PackedStringArray :
...
static func getGenerator(scenePath) -> Array[Node]:
	...
	if packedScene == null:
		print("could not load scene path:",scenePath)
		breakpoint
		return [null,null]
...
static func createEntity(entityName : StringName ,params : Dictionary,entityEntry,specificCacheParent,resourceManager : Node,entityLoader : Node = null):
	var hidden = false
	var ent = null
	...
	if entityEntry.has("creationFunction"):
		...
		if entityEntry.has("postCreationFunction"):
			if entityLoader.get_method_argument_count(entityEntry["postCreationFunction"]) == 3:
				var funcToBind := Callable(entityLoader, entityEntry["postCreationFunction"])
				postCreationFunction = funcToBind.bind(entityEntry,params)
			else:
				var funcToBind := Callable(entityLoader, entityEntry["postCreationFunction"])
		...
		if !entityEntry["sourceScene"].is_empty():
			var scaleFactor = Vector3.ZERO
			
			
			ent = ENTG.sceneBasedInstance(entityEntry["sourceScene"],resourceManager,entityLoader,parent.scaleFactor,postCreationFunction,specificCacheParent)
			if ent == null:
				breakpoint
	else:
		return
...
static func getPlayers(tree):
	return tree.get_nodes_in_group("player")

static func createFromCode(entityCreator : Node,entityEntry : Dictionary,function : StringName,params : Dictionary = {}) -> Node:

	var par : Node =entityCreator.get_parent()
	var destPath = "res://game_imports/"+par.gameName +"/"+entityEntry["name"]+".tscn"
	var scene : Node3D = entityCreator.call(function,entityEntry,params)
	
	if scene == null:
		return null
	
	scene.set_meta("gameName",par.gameName.to_lower())
	scene.set_meta("entityName",entityEntry["name"].to_lower())
	...
	if generator != null:
		...
		if "dependantChildren" in generator:
			for i in generator.dependantChildren:
				...
				var dependantInstanced = sceneBasedInstance(i,resourceManager)
...
static func createMidiPlayer(soundFontPath : String= "", loop :bool = true):
	if !doesFileExist("res://addons/midi/MidiPlayer.tscn"):
		return
...
static func createFontImage(characters : Dictionary) -> Image:
	...
	var img : Image = Image.create(totalW,maxY,false,Image.FORMAT_RGB8)
...
static func createAndInitializeLoader(tree :SceneTree,configName : String,param : Array, extraParam : Array = []):
	...
	for i in loaders:
		...
		for gameName in inst.getConfigs():
			...
			if gameName == configName:
				...
				if param.is_empty():
					param = ENTG.searchForFilesGivenLoder(inst,configName)
				
				if inst.initialize.get_argument_count() == 4:
					inst.initialize(param,configName,configName,extraParam)
				else:
...
static func getAllModels(tree : SceneTree,gameName : String):
	if !tree.has_meta("loaders"):
		return
		
	var loaders = tree.get_meta("loaders")
	
	var ret = []
	
	for loader in loaders.values():
		ret += loader.getAllModels()
...
static func getAllFlat2(path: String, filter: PackedStringArray = [],ingoreDir = "",maxDepth = -1) -> Array:
	...
	while stack.size() > 0:
		...
		var entries = ENTG.allInDirectory2(current_path, filter)
...
static func findFiles(files,hints,ext,androidTest):
	...
	if androidTest:
		if DirAccess.dir_exists_absolute(t[1]):
			allFiles = ENTG.allInDirectory2(t[1], ext)
			breakpoint
		
		
	allFiles = getAllFlat2("res://",ext,["/game_imports","/addons","/dbg"])
	
	print("elpased time:",Time.get_ticks_msec()-a)
	for f in allFiles:
...
static func searchForFilesGivenLoder(loader,configName):
	var reqs = loader.getReqs(configName)
	...
	for rIdx in reqs.size():
		...
		if !fileNames.is_empty():
			foundFiles = searchForFiles(fileNames,ext,hints)
		
		if !foundFiles.is_empty():
			ret.append_array(foundFiles)
	
	return ret
	
	
static func searchForFiles(fileNames,ext,hints):
	var exts = []
	...
	if OS.has_feature("android"):
		...
		var test = ENTG.findFiles(fileNames,hints,exts,OS.has_feature("android"))
		breakpoint
		
	return ENTG.findFiles(fileNames,hints,exts,false)

static func getCtexPath(path) -> String:
	var fileName = path.get_basename().get_file()
	var fileHash = fileName + "-" + path.md5_text()
	
	return "res://.godot/imported/" + fileHash.split("-",false)[0] + ".png-"  + fileHash.split("-")[1] + ".ctex"
