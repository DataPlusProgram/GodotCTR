...
@export var spriteBased : bool = false
...
func _physics_process(delta):
	...
	if oscillationSpeed > 0:
		...
		model.position.y = oscillationHeight*sin(itt*oscillationSpeed)
			
		if spriteBased:
			model.position.y  += model.mesh.size.y/2.0
...
func pickedUp(body):
	...
	for idx in giveString.size():
		...
		if spriteBased:
			dict = {"giveName":giveString[idx],"giveAmount":giveAmount[idx],"persistant":persistant[idx],"sprite":model.mesh.material.get_shader_parameter("texture_albedo")}
		else:
			dict = {"giveName":giveString[idx],"giveAmount":giveAmount[idx],"persistant":persistant[idx]}
