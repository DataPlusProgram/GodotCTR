...
func _ready() -> void:
	...
	if get_parent().has_meta("height"):
		...
		height = EGLO.getCollisionShapeHeight(get_parent().get_node("CollisionShape3D"))
	elif get_parent().has_node("Area3D"):
		if get_parent().get_node("Area3D").has_node("CollisionShape3D"):
			height = EGLO.getCollisionShapeHeight($"../Area3D/CollisionShape3D")
...
func _physics_process(delta):
	...
	if touching != null:
		...
		if touching.process_mode != PROCESS_MODE_DISABLED:
			enabled = false
			return
